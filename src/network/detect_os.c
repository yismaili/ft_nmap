...
OSFingerprint os_signatures[] = {
    ...
    {"Windows Server 2019", 128, 8192, 2},
    {"Windows Server 2016", 128, 8192, 2},
    {"Windows 7/8", 128, 8192, 2},
    {"Windows XP", 128, 65535, 2},

    {"macOS Sonoma", 64, 65535, 3},
    {"macOS Ventura", 64, 65535, 3},
    {"macOS Monterey", 64, 65535, 3},
    {"macOS Big Sur", 64, 65535, 3},
    {"macOS Catalina", 64, 65535, 3},

    {"FreeBSD 13.x", 64, 65535, 4},
    {"FreeBSD 12.x", 64, 65535, 4},
...
const char *detect_os(const char *target, int config_timeout) {
    ...
    for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
        ...
        if (inet_ntop(AF_INET, &addr->sin_addr, source_ip, INET_ADDRSTRLEN) != NULL &&
    ...
    for (int i = 0; i < signatures_count; i++) {
        ...
        if (abs(probe_results.ttl - os_signatures[i].initial_ttl) <= 16)
            score += 2;
        if (abs(probe_results.window_size - os_signatures[i].tcp_window_size) <= 512)
            score += 3;
